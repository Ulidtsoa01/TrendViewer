...
function TickerGeneral() {
  ...
  const handleEditTickerDelete = () => {
    removeTicker(tickerInfo.name, tickers, dispatch);
    setGeneralMode('general');
  };

  return (
    <Tabs id="ticker_page" activeKey={tabKey} onSelect={(k) => setTabKey(k)} className="mb-3">
      <Tab eventKey="ticker" title={tickerInfo ? tickerInfo.name : 'General'}>
        {tickerInfo && generalMode === 'general' && (
          <TickerFields
            tickerInfo={tickerInfo}
            onEditTickerClicked={handleEditTicker}
            onModifySettingClicked={handleModifySettings}
          />
        )}
        {tickerInfo && generalMode === 'editticker' && (
          <TickerEdit
            tickerInfo={tickerInfo}
            onCancelClicked={handleEditTickerCancel}
            onDeleteClicked={handleEditTickerDelete}
          />
        )}
        {tickerInfo && generalMode === 'modifysetting' && (
          <EditSettings tickerInfo={tickerInfo} onCancelClicked={handleEditTickerCancel} />
        )}
      </Tab>
      <Tab eventKey="activities" title="Activities" disabled={!tickerInfo}>
        {tickerInfo && <TickerActivities tickerInfo={tickerInfo} />}
      </Tab>
      <Tab eventKey="journals" title="Journals" disabled={!tickerInfo}>
        {tickerInfo && <TickerJournals tickerInfo={tickerInfo} />}
      </Tab>
      <Tab eventKey="quotes" title="Quotes" disabled={!tickerInfo}>
        {tickerInfo && <TickerQuotes tickerInfo={tickerInfo} />}
      </Tab>
    </Tabs>
  );
}
