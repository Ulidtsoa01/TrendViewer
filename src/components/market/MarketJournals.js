...
function MarketJournals(props) {
    ...
    const handleRowClick = (e) => {
        ...
        const cj = journalList.find((j) => j.id === Number(e.target.id));
        if (cj) {
    ...
    const toGeneralMode = (journal) => {
        if (journal) {
            const cj = journalList.find((j) => j.id === journal.id);
            if (cj) {
        ...
        <>
            {mode === 'general' &&
                <Navbar expand="lg" className="bg-body-tertiary">
                    <Container fluid>
                        <Navbar.Brand as={Link} to="#">Market Assessment</Navbar.Brand>
                        <Navbar.Collapse id="navbarMarketGeneral">
                            <Form className="d-flex">
                                <Button variant="primary" onClick={handleAdd}>Add Assessment</Button>
                            </Form>
                        </Navbar.Collapse>
                    </Container>
                </Navbar>
            ...
            {mode === 'general' &&
                <SimpleTable data={journalList} columns={columns} keyField='id' onRowClick={handleRowClick} />
            }
            {mode === 'editjournal' &&
                <MarketJournalEdit journal={clickedJournal} toGeneralMode={toGeneralMode} />
            }
            {mode === 'addjournal' &&
                <MarketJournalEdit toGeneralMode={toGeneralMode} />
            }
        </>
    );
}
