...
const columns = [
    { title: 'Account Name', field: 'name', type: 'string', formatter: (r, v) => v === 'Summary' ? <b>Summary</b> : <Link to={'/account/' + r.id}>{v}</Link> },
    ...
    { title: 'Gain', field: 'gain', type: 'number', formatter: (r, v) => r.name === 'Summary' ? <b>{v.toLocaleString()}</b> : v.toLocaleString() },
...
function AccountsSummary(props) {
    ...
    return (
        <SimpleTable data={accounts} columns={columns} keyField='id' />
    );
}
