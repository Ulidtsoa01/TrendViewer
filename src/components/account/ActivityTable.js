...
function ActivityTable(props) {
    ...
    const handleActivityClick = (e) => {
        ...
        const ca = activityList.find((j) => j.id === Number(e.target.id));
        if (ca) {
    ...
    const columns = [
        { title: 'Date', field: 'date', type: 'date', formatter: (r, v) => <a href="#" id={r.id} onClick={handleActivityClick}>{(new Date(v)).toISOString().split('T')[0]}</a> },
        ...
        <>
            {mode === 'home' &&
                <Navbar expand="lg" className="bg-body-tertiary">
                    <Container fluid>
                        <Navbar.Brand as={Link} to="#">{account ? account.name : 'Trade Activities'}</Navbar.Brand>
                        <Navbar.Collapse id="navbarActivities">
                            <Form className="d-flex">
                                ...
                                {account &&
                                    <Button variant="primary" className={styles.toolbarButton} onClick={handleAddClick}>Create</Button>
                                }
                            </Form>
                        </Navbar.Collapse>
                    </Container>
            ...
            {mode === 'addactivity' &&
                <ActivityEdit account={account} onCancel={handleCancel}/>
            }
            {mode === 'editactivity' &&
                <ActivityEdit account={account} activity={clickedActivity} onCancel={handleCancel}/>
            }
        </>
    )
}
