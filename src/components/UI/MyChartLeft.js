    ...
    return (
        <div className={styles.rootDiv}>
            ...
            <div className={styles.indexRow}>
                <Button variant="primary" disabled={!stockList || index <= 0} onClick={toPrevious}>Previous</Button>
                <h5>{indexStr}</h5>
                <Button variant="primary" disabled={!stockList || index >= stockList.length - 1} onClick={toNext}>Next</Button>
            </div>
            <div>
                <p>Last Price: {data.dquote.last} on {dquoteDateString}</p>
                <p>Last Quote: {lastQuote.close} on {lastQuoteDateString}</p>
            </div>
            ...
            <div className={styles.externalLinksRow}>
                <a href={"http://localhost:8080/#/ticker/id/" + data.tickerId} target='_blank' rel='noreferrer'>Ticker</a>
                <a href={"https://seekingalpha.com/symbol/" + data.tickerName} target='_blank' rel='noreferrer'>Seeking Alpha</a>
                <a href={"https://bigcharts.marketwatch.com/quickchart/quickchart.asp?symb=" + data.tickerName} target="_blank" rel='noreferrer'>Big Charts</a>
                <a href={"https://finance.yahoo.com/quote/" + data.tickerName} target="_blank" rel='noreferrer'>Yahoo</a>
            </div>
...
export async function action({ request, params }) {
    ...
    let url = 'http://localhost:8080/rest/tinfo';
